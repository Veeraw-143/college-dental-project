...
def booking(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Send admin notification (don't let email failure break the booking)
            email_sent = False
            email_error = None
            try:
                email_sent = new_booking.send_admin_notification(request=request)
                if not email_sent:
                    logger.warning(f'Admin notification could not be sent for booking {new_booking.pk}, but booking was created successfully')
                    email_error = 'Could not notify admin (email service issue)'
            except Exception as e:
                logger.warning(f'Exception while sending admin notification for booking {new_booking.pk}: {e}')
                email_error = str(e)
        ...
        if email_error:
            payload['email_warning'] = f'Booking confirmed but: {email_error}'
        if request.headers.get('x-requested-with') == 'XMLHttpRequest':
            return JsonResponse(payload)
...
# ============= CHATBOT VIEWS =============
# Robust production-ready chatbot API implementation
from project.chatbot_api import chatbot_message_api, get_faq_list
